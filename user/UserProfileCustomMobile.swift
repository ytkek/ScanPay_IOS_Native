...
class UserProfileCustomMobile: UIViewController ,UITextFieldDelegate{
    ...
    @IBAction func save(_ sender: UIButton) {
        ...
        if Reachability.isConnectedToNetwork(){
        let url2 = URL(string: "https://www.myscanpay.com/V5/mobile_native_api/PostUserProfile_MobileNumber.aspx")
                     guard let requestUrl = url2 else { fatalError() }
                     // Prepare URL Request Object
                     var request = URLRequest(url: requestUrl)
                     request.httpMethod = "POST"
            ...
            let Encryptedvalue = DiscoveryCell.aesEncrypt(text : value,key: "@McQfTjWnZq4t7w!")
             ...
             let postStringencoding = Encryptedvalue.addingPercentEncoding(withAllowedCharacters: .alphanumerics)
                      ...
                      if let myString = MobileNumber {
                            let postString = "LoginID=\(phoneinput)&MobileNumber=\(myString)&Token=\(postStringencoding ?? "")";
